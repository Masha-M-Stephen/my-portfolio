        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((item) => {
            ...
            return (
              <Wrapper
                key={item.title}
                {...(isComingSoon ? {} : { href: item.link, target: "_blank", rel: "noopener noreferrer" })}
              ...
              >
                <div className="relative h-48 w-full overflow-hidden">
                  ...
                  {isComingSoon && (
                    <div className="absolute right-3 top-3 rounded-full bg-purple-primary/80 px-3 py-1 text-xs font-medium text-white">
                      Coming Soon
                    </div>
                  )}
                </div>
